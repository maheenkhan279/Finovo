...
def random_event():
    ...
    elif chance < 0.99:  # 1% chance of super positive event
        desc, amount = random.choice(SUPER_POSITIVE_EVENTS)
        return {"type": "super_positive", "desc": desc, "amount": amount}
    else:
        return {"type": "none", "desc": "No random event", "amount": 0}

@moderate_bp.route('/')
def moderate_game():
    """Start the budget balancer game"""
    ...
    return render_template('games/moderate_budget.html', 
                         game_state=game_state,
                         spending_options=SPENDING_OPTIONS,
                         total_months=TOTAL_MONTHS)

@moderate_bp.route('/receive_income', methods=['POST'])
def receive_income():
...
@moderate_bp.route('/simulate_month', methods=['POST'])
def simulate_month():
    ...
    elif event['type'] in ['positive', 'super_positive']:
        game_state['player_balance'] += event['amount']
...
@moderate_bp.route('/next_month', methods=['POST'])
def next_month():
    """Move to next month"""
    game_state = session.get('game_state', {})
...
@moderate_bp.route('/update_choice', methods=['POST'])
def update_choice():
...
@moderate_bp.route('/update_settings', methods=['POST'])
def update_settings():
    """Update game settings"""
    data = request.json
    game_state = session.get('game_state', {})
    
    if game_state.get('current_month') == 1 and game_state.get('phase') == 'await_income':
        game_state['starting_balance'] = data.get('starting_balance', 1000)
        game_state['monthly_income'] = data.get('monthly_income', 2500)
        game_state['player_balance'] = game_state['starting_balance']
        session['game_state'] = game_state
    
    return jsonify(game_state)

@moderate_bp.route('/reset', methods=['POST'])
def reset_game():
    """Reset the game"""
    session['game_state'] = {
        'player_balance': 1000,
